...
//    	boolean isCoolDown = false;
    	Thread t1 =new Thread(()->{
			while (!GameController.isGameEnded()) {
				try {
					if (GameController.getKeyboardController().isPPressed() && !isUseSpecialAbility()) {
						isUseSpecialAbility = true;
						/*
						 * ====================FILL CODE============================ Add something to
						 * activate ability for 1 second and then cool down (do not allow to activate)
						 * for 5 second
						 */
